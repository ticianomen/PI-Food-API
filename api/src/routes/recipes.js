...
router.get('/recipes/:id', async function(req,res){
    ...
    try{
        ...
        let arrDB ={
            ...
            ingredients: dietDB.ingredients,
        }
        res.json(arrDB)
    }
    catch{
    ...
    try{
        if(name){          
            ...
            if(recipesDB.length>99){
                ...
                const response = await fetch(`https://api.spoonacular.com/recipes/complexSearch?titleMatch=${name}&number=${numbers}&addRecipeInformation=true&apiKey=${MY_API_LU}`)
            ...
            if(arrDB.length>99){
                ...
                try{
                    ...
                    const response = await fetch(`https://api.spoonacular.com/recipes/complexSearch?number=${numbers}&addRecipeInformation=true&apiKey=${MY_API_LU}`)
...
router.post('/recipe', async function(req,res){
    const {title,summary,spoonacularScore,healthScore,instructions,diets,image,dishTypes,cuisines,ingredients} = req.body
    
    let [recipe] = await Recipe.findOrCreate({where:{title,summary,spoonacularScore,healthScore,instructions,ingredients,image,dishTypes,cuisines}})
